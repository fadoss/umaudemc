...
def _actionParse(aprop):
	...
	text = str(aprop)
	typeAnnot = text.find(').@ActionList@')

	return text[1:typeAnnot] if text[0] == '(' and typeAnnot > 0 else text
...
def _formula2List(form, *extra_args):
	...
	propSort, variableSort, labels = extra_args

	if form.getSort().leq(propSort):
		return ['Prop', str(form)]

	elif form.getSort().leq(variableSort):
		return ['Var', str(form)]
...
class Parser:
	...
	def set_module(self, module, metamodule=None):
		...
		self.ruleLabels = ' '.join([f"'{label}" for label in self.labels]) \
				  if self.labels != [] else 'nil'
	...
	def parse(self, formula):
		...
		parsedFormula  = extmod.parseTerm(formula, formula_kind)
		# a copy can be made to preserve the original

		if parsedFormula is None:
			usermsgs.print_error('The given temporal formula cannot be parsed.')
			return None, None

		logic = self.deduce_logic(extmod, parsedFormula)
		...
		return _formula2List(parsedFormula, prop_sort, mcvariable_sort, set(self.labels)), logic
